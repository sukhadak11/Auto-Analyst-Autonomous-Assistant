...
def select_and_generate_visualizations(
    ...
    for col in cat_cols + num_cols:
        if col.lower().replace("_", " ") in question_lower or col.lower() in question_lower:
            mentioned_col = col
            break

    if mentioned_col and target_col in df.columns:
        plt.figure(figsize=(8, 5))
        pd.crosstab(df[mentioned_col], df[target_col]).plot(kind="bar")
        plt.xlabel(mentioned_col)
        plt.ylabel("Count")
        plt.title(f"{target_col} by {mentioned_col}")
        plt.tight_layout()
        path = f"{output_dir}/grouped_{mentioned_col}_{job_id}.png"
        plt.savefig(path, dpi=150)
        plt.close()
        generated_charts.append(("grouped_comparison", path))
        explanations.append(make_explanation(
            action=f"Generated a grouped bar chart of '{target_col}' by '{mentioned_col}'",
            reason=f"The question specifically asked to compare {target_col} by {mentioned_col}.",
            alternative_considered="Generic target distribution chart only",
            why_not_chosen="The question asked for a specific breakdown, not just an overall count.",
            expected_impact=f"Report includes a chart showing {target_col} split by {mentioned_col}.",
            learning_note="Grouped bar charts are the standard way to compare a categorical outcome across another category's subgroups.",
            confidence="High",
        ))
